...
<body>
        ...
        <div class="modal-content">
            ...
            <form id="add-chord-form">
                ...
                <div id="barre-details" style="display: none; border-left: 2px solid var(--primary-color); padding-left: 15px;">
                    <div class="form-group"><label for="barre-fret">Barre Fret:</label><input type="number" id="barre-fret" min="1"></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            addChordForm.addEventListener('submit', async e => { e.preventDefault(); addChordError.textContent = ''; const frets = Array.from(document.querySelectorAll('.fret-input')).map(input => parseInt(input.value, 10)); const barres = document.getElementById('has-barre').checked ? [{ fret: parseInt(document.getElementById('barre-fret').value, 10), fromString: parseInt(document.getElementById('barre-from').value, 10), toString: parseInt(document.getElementById('barre-to').value, 10) }] : []; const payload = { name: document.getElementById('new-chord-name').value, title: document.getElementById('new-chord-title').value, frets, barres }; try { const response = await fetch('/api/chords', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) throw new Error(await response.text()); addChordModalEl.style.display = 'none'; await initializeApp(); } catch (error) { addChordError.textContent = `Error: ${error.message}`; } });
